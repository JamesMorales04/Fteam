                ...
                @if (sizeOf($orders['orders']) > 0 )
                    @foreach ($orders['orders'] as $order)
                        <div class="card">
                            ...
                            <strong><div class="card-header">{{ __('messages.order') }} {{ $order->getId() }}</div></strong> 
                            ...
                            <div class="card-body">
                                ...
                                @endforeach
                                <label class="col-ld-8 "><strong>{{ __('messages.total') }}</strong>: {{ $order->getTotal()}}</label><br />
                                <label class="col-ld-8 "><strong>{{__('messages.creationDate')  }}</strong>: {{ $order->getRegisterDate()}}</label><br />
                            </div>
                        </div><br />
